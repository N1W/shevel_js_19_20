$(document).ready(function(){
	...
	var skillsArr = [];
	var newArray = _.forEach(jsonData, function(value, key) {
		skillsArr = skillsArr.concat(value.skills);
	});
	newArray = _.uniq(skillsArr);
	console.log('skills array = ', newArray.sort());
//----------------------------------------------------------------
	var friendsArr = [];
	var newFrindsArr = _.forEach(jsonData, function(value, key) {
		_.forEach(value.friends, function(value, key) {
			friendsArr = friendsArr.concat(value.name);
		});
	});
	friendsArr = _.uniq(friendsArr);
	console.log('friend array = ', friendsArr.sort());
//-----------------------------------------------------------------
	var nameArr1 = [];
	var nameArr2 = [];
	var nameArr3 = [];
	var nameArrResult = [];
	var newNameArr = _.forEach(jsonData, function(value, key) {

		if (value.friends.length == 1) {
			nameArr1 = nameArr1.concat(value.name);
		}
		if (value.friends.length == 2) {
			nameArr2 = nameArr2.concat(value.name);
		}
		if (value.friends.length == 3) {
			nameArr3 = nameArr3.concat(value.name);
		}
	});
		nameArrResult = nameArrResult.concat(nameArr3);
		nameArrResult = nameArrResult.concat(nameArr2);
		nameArrResult = nameArrResult.concat(nameArr1);
		console.log('names array = ', nameArrResult);
